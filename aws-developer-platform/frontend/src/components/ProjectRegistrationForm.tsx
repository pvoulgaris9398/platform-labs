...
const resourceTypes = ['s3', 'lambda', 'dynamodb'] as const;
const projectSchema = z.object({
  ...
  allowed_resource_types: z.array(z.enum(resourceTypes)).min(1, 'Select at least one resource type.'),
...
export function ProjectRegistrationForm({ identity }: ProjectRegistrationFormProps): React.JSX.Element {
  ...
  const { control, register, handleSubmit, reset, formState: { errors } } = useForm<ProjectFormValues>({
    ...
    defaultValues: {
      ...
      allowed_environments: ['dev', 'uat'], allowed_resource_types: [...resourceTypes],
  ...
  return <Stack component="form" spacing={2} onSubmit={handleSubmit(submit)} noValidate>
    ...
    <Controller name="allowed_resource_types" control={control} render={({ field }) => <FormControl error={Boolean(errors.allowed_resource_types)}><InputLabel id="resource-types-label">Allowed resource types</InputLabel><Select {...field} labelId="resource-types-label" multiple input={<OutlinedInput label="Allowed resource types" />} renderValue={(selected: ResourceType[]) => selected.join(', ')}>{resourceTypes.map((value) => <MenuItem key={value} value={value}><Checkbox checked={field.value.includes(value)} /><ListItemText primary={value} /></MenuItem>)}</Select></FormControl>} />
